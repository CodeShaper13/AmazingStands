...
public class AmazingStandEventHandler {
	...
	public AmazingStandEventHandler() {
		this.methodSetShowArms = ReflectionHelper.findMethod(EntityArmorStand.class, "setShowArms", "func_175413_k", boolean.class);
		this.methodSetSmall = ReflectionHelper.findMethod(EntityArmorStand.class, "setSmall", "func_175420_a", boolean.class);
	}
	...
	public void attackEntity(AttackEntityEvent event) {
		if (event.getTarget() instanceof EntityArmorStand) {
			...
			if(player.isSneaking()) {				
				...
				// Switch what hand is active.
				if(heldStack == ItemStack.EMPTY) {
					ItemStack mainTemp = armorStand.getHeldItemMainhand();
					armorStand.setHeldItem(EnumHand.MAIN_HAND, armorStand.getHeldItem(EnumHand.OFF_HAND));
					armorStand.setHeldItem(EnumHand.OFF_HAND, mainTemp);
				}				
				// Remove base plate.
				...
				// Add/remove arms.
					...
					if (!armorStand.getShowArms() && heldStack.getCount() >= (player.capabilities.isCreativeMode ? 1 : 2)) {
				...
				// Place banner on head.
						...
						&& (heldItem.isValidArmor(heldStack, EntityEquipmentSlot.HEAD, armorStand)
								|| heldItem == Items.BANNER)) {
				...
				// Remove no gravity and/or invisibility.
					...
					if (consumedMilk && !player.capabilities.isCreativeMode) {
				...
				// Change slab.
					...
					PacketManager.INSTANCE.sendToDimension(
							new PacketBasePlateStack(armorStand.getEntityId(), cap.getStandStack()),
							armorStand.dimension);
					somethingHappened = true;
				}
				// Make invisible.
	...
	public void attachCapability(AttachCapabilitiesEvent<Entity> event) {
		if (event.getObject() instanceof EntityArmorStand) {
			event.addCapability(new ResourceLocation(AmazingStands.MOD_ID, "armorStandData"),
					new AmazingStandDataProvider());
		}
	}
	...
	public void livingDeath(LivingDeathEvent event) {
		if (event.getEntity() instanceof EntityArmorStand) {
			...
			Block.spawnAsEntity(e.world, new BlockPos(e),
					e.getCapability(AmazingStandDataProvider.CAPABILITY, null).getStandStack());
		}
	}

	// Copied from ArmorStand.playBrokenSound
	private void playSoud(EntityArmorStand armorStand) {
		armorStand.world.playSound((EntityPlayer) null, armorStand.posX, armorStand.posY, armorStand.posZ,
				SoundEvents.ENTITY_ARMORSTAND_BREAK, armorStand.getSoundCategory(), 1.0F, 1.0F);
	}
